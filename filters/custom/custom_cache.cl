...
typedef enum RandomType {
    ...
    R_Boss,
		...
		R_Pocket, //  pocket pack card type, > 60% -> energy
		R_PocketCardMatch,
		R_MasterBall,
		R_Joker_Safari,
		R_Joker_LegPok,
...
text type_str(int x) {
    switch(x) {
        ...
        case R_Boss:                     return init_text("boss", 4);
				...
				case R_Energy :                  return init_text("Energy", 6); 
...
text source_str(int x) {
    switch(x) {
        ...
        case S_Cartomancer:      return init_text("car", 3);
				case S_Pocket     :      return init_text("poc", 3);
				case S_MasterBall     :  return init_text("mas", 3);
        default:               return init_text("", 0);
    }
}
